  ...
  methods: {
    initData: function () {
      window.luckysheet.create({
        ...
        lang: 'zh',
        plugins: ['chart'],
        loadUrl: "api/load/" + this.doc_id,
        loadSheetUrl: "api/loadSheet/" + this.doc_id,
        updateUrl: "ws://192.168.101.4:9999/ws/" + Math.round(Math.random() * 100) + "/" + this.doc_id
      });
      console.log(window.luckysheet)

      // this.request.post("/api/load/" + this.doc_id).then(res => {
      //   this.doc = res.data.all
